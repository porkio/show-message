 ...
 */
export default function showMessage({ title, icon, duration = 2500, cb }) {
	if (!title) {
		throw new Error('title can not be null')
	}
	const _this = {}
	_this.title = title
	_this.icon = icon ? icon.toLowerCase() : null
	icon && (_this.icon = icon.toLowerCase())
	_this.messageBox = document.createElement('div')
	_this.messageTitle = document.createElement('span')
	_this.iconsMaping = new Map()
		.set(
			'success',
	...
	_this._render = function () {
		_this.messageTitle.innerText = _this.title
		_this.messageBox.style = `
            display: flex;
            align-items: center;
            position: fixed;
            top: 50%;
            ...
            z-index: 99999;
        `
		_this.messageTitle.style = `
            white-space: nowrap;
            text-overflow: ellipsis;
            overflow: hidden;
        `

		if (_this.icon && _this.iconsMaping.get(_this.icon)) {
			_this.messageBox.innerHTML = _this.iconsMaping.get(_this.icon)
			_this.messageBox.appendChild(_this.messageTitle)
		} else {
			_this.messageBox.appendChild(_this.messageTitle)
		}

		document.body.appendChild(_this.messageBox)

		setTimeout(() => {
			document.body.removeChild(_this.messageBox)
			_this.cb && _this.cb()
		}, _this.duration)
	}
